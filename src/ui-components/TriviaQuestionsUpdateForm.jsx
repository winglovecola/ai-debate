...
import { getTriviaQuestions } from "../graphql/queries";
import { updateTriviaQuestions } from "../graphql/mutations";
const client = generateClient();
export default function TriviaQuestionsUpdateForm(props) {
  const {
    id: idProp,
    triviaQuestions: triviaQuestionsModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = triviaQuestionsRecord
      ? { ...initialValues, ...triviaQuestionsRecord }
      : initialValues;
    setCategory(cleanValues.category);
    setType(cleanValues.type);
    setDifficultyLvl(cleanValues.difficultyLvl);
    setQuestion(cleanValues.question);
    setAnswerData(cleanValues.answerData);
    setAnsChoices(cleanValues.ansChoices);
    setAnsEncData(cleanValues.ansEncData);
    setRelatedShowsData(cleanValues.relatedShowsData);
    setStatus(cleanValues.status);
    setErrors({});
  };
  const [triviaQuestionsRecord, setTriviaQuestionsRecord] = React.useState(
    triviaQuestionsModelProp
  );
  React.useEffect(() => {
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getTriviaQuestions.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getTriviaQuestions
        : triviaQuestionsModelProp;
      setTriviaQuestionsRecord(record);
    };
    queryData();
  }, [idProp, triviaQuestionsModelProp]);
  React.useEffect(resetStateValues, [triviaQuestionsRecord]);
  const validations = {
    category: [],
    type: [],
    difficultyLvl: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          ...
          type: type ?? null,
          difficultyLvl: difficultyLvl ?? null,
          question: question ?? null,
          answerData: answerData ?? null,
          ansChoices: ansChoices ?? null,
          ansEncData: ansEncData ?? null,
        ...
        try {
          ...
          await client.graphql({
            query: updateTriviaQuestions.replaceAll("__typename", ""),
            variables: {
              input: {
                id: triviaQuestionsRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "TriviaQuestionsUpdateForm")}
      {...rest}
    >
      <TextField
        label="Category"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || triviaQuestionsModelProp)}
