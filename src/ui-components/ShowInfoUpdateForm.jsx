...
import { getShowInfo } from "../graphql/queries";
import { updateShowInfo } from "../graphql/mutations";
const client = generateClient();
export default function ShowInfoUpdateForm(props) {
  const {
    id: idProp,
    showInfo: showInfoModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = showInfoRecord
      ? { ...initialValues, ...showInfoRecord }
      : initialValues;
    setPrevShowId(cleanValues.prevShowId);
    setPrevShowBroadcastTime(cleanValues.prevShowBroadcastTime);
    setLatestShowId(cleanValues.latestShowId);
    setLatestShowBroadcastTime(cleanValues.latestShowBroadcastTime);
    setNextShowId(cleanValues.nextShowId);
    setNextShowBroadcastTime(cleanValues.nextShowBroadcastTime);
    setUpcomingShowsJsonData(cleanValues.upcomingShowsJsonData);
    setUpcomingShowsJsonDataUpdateTime(
      cleanValues.upcomingShowsJsonDataUpdateTime
    );
    setErrors({});
  };
  const [showInfoRecord, setShowInfoRecord] = React.useState(showInfoModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getShowInfo.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getShowInfo
        : showInfoModelProp;
      setShowInfoRecord(record);
    };
    queryData();
  }, [idProp, showInfoModelProp]);
  React.useEffect(resetStateValues, [showInfoRecord]);
  const validations = {
    prevShowId: [],
    prevShowBroadcastTime: [],
    latestShowId: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          prevShowId: prevShowId ?? null,
          prevShowBroadcastTime: prevShowBroadcastTime ?? null,
          latestShowId: latestShowId ?? null,
          latestShowBroadcastTime: latestShowBroadcastTime ?? null,
          nextShowId: nextShowId ?? null,
          nextShowBroadcastTime: nextShowBroadcastTime ?? null,
          upcomingShowsJsonData: upcomingShowsJsonData ?? null,
          upcomingShowsJsonDataUpdateTime:
            upcomingShowsJsonDataUpdateTime ?? null,
        };
        ...
        try {
          ...
          await client.graphql({
            query: updateShowInfo.replaceAll("__typename", ""),
            variables: {
              input: {
                id: showInfoRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "ShowInfoUpdateForm")}
      {...rest}
    >
      <TextField
        label="Prev show id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || showInfoModelProp)}
