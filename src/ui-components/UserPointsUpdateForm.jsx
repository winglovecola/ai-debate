...
import { getUserPoints } from "../graphql/queries";
import { updateUserPoints } from "../graphql/mutations";
const client = generateClient();
export default function UserPointsUpdateForm(props) {
  const {
    id: idProp,
    userPoints: userPointsModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = userPointsRecord
      ? { ...initialValues, ...userPointsRecord }
      : initialValues;
    setTrReferralBonus(cleanValues.trReferralBonus);
    setTrLoyaltyBonus(cleanValues.trLoyaltyBonus);
    setTrLoyaltyTotal(cleanValues.trLoyaltyTotal);
    setTrLoyaltyBonusResetedTime(cleanValues.trLoyaltyBonusResetedTime);
    setTrTotalPoints(cleanValues.trTotalPoints);
    setTrScoreTotal(cleanValues.trScoreTotal);
    setErrors({});
  };
  const [userPointsRecord, setUserPointsRecord] =
    React.useState(userPointsModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getUserPoints.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getUserPoints
        : userPointsModelProp;
      setUserPointsRecord(record);
    };
    queryData();
  }, [idProp, userPointsModelProp]);
  React.useEffect(resetStateValues, [userPointsRecord]);
  const validations = {
    trReferralBonus: [],
    trLoyaltyBonus: [],
    trLoyaltyTotal: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          trReferralBonus: trReferralBonus ?? null,
          trLoyaltyBonus: trLoyaltyBonus ?? null,
          trLoyaltyTotal: trLoyaltyTotal ?? null,
          trLoyaltyBonusResetedTime: trLoyaltyBonusResetedTime ?? null,
          trTotalPoints: trTotalPoints ?? null,
          trScoreTotal: trScoreTotal ?? null,
        };
        ...
        try {
          ...
          await client.graphql({
            query: updateUserPoints.replaceAll("__typename", ""),
            variables: {
              input: {
                id: userPointsRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "UserPointsUpdateForm")}
      {...rest}
    >
      <TextField
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || userPointsModelProp)}
