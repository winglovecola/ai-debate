...
import { getUserReward } from "../graphql/queries";
import { updateUserReward } from "../graphql/mutations";
const client = generateClient();
export default function UserRewardUpdateForm(props) {
  const {
    id: idProp,
    userReward: userRewardModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = userRewardRecord
      ? { ...initialValues, ...userRewardRecord }
      : initialValues;
  ...
  const [userRewardRecord, setUserRewardRecord] =
    React.useState(userRewardModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getUserReward.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getUserReward
        : userRewardModelProp;
      setUserRewardRecord(record);
    };
    queryData();
  }, [idProp, userRewardModelProp]);
  React.useEffect(resetStateValues, [userRewardRecord]);
  const validations = {
    trReferralBonus: [],
    trLoyaltyBonus: [],
    trLoyaltyBonusResetTime: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateUserReward.replaceAll("__typename", ""),
            variables: {
              input: {
                id: userRewardRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "UserRewardUpdateForm")}
      {...rest}
    >
      <TextField
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || userRewardModelProp)}
