...
import { getScorePool } from "../graphql/queries";
import { updateScorePool } from "../graphql/mutations";
const client = generateClient();
export default function ScorePoolUpdateForm(props) {
  const {
    id: idProp,
    scorePool: scorePoolModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = scorePoolRecord
      ? { ...initialValues, ...scorePoolRecord }
      : initialValues;
    setType(cleanValues.type);
    setCategory(cleanValues.category);
    setTitle(cleanValues.title);
    setLoyaltyBonusPoints(cleanValues.loyaltyBonusPoints);
    setLoyaltyBonusPointsCumulative(cleanValues.loyaltyBonusPointsCumulative);
    setScoreData(cleanValues.scoreData);
    setStatus(cleanValues.status);
    setErrors({});
  };
  const [scorePoolRecord, setScorePoolRecord] =
    React.useState(scorePoolModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getScorePool.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getScorePool
        : scorePoolModelProp;
      setScorePoolRecord(record);
    };
    queryData();
  }, [idProp, scorePoolModelProp]);
  React.useEffect(resetStateValues, [scorePoolRecord]);
  const validations = {
    type: [],
    category: [],
    title: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          ...
          title: title ?? null,
          loyaltyBonusPoints: loyaltyBonusPoints ?? null,
          loyaltyBonusPointsCumulative: loyaltyBonusPointsCumulative ?? null,
        ...
        try {
          ...
          await client.graphql({
            query: updateScorePool.replaceAll("__typename", ""),
            variables: {
              input: {
                id: scorePoolRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "ScorePoolUpdateForm")}
      {...rest}
    >
      <TextField
        label="Type"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || scorePoolModelProp)}
