...
export default function UserAccountUpdateForm(props) {
  const {
    id: idProp,
    userAccount: userAccountModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = userAccountRecord
      ? { ...initialValues, ...userAccountRecord }
      : initialValues;
    setIp(cleanValues.ip);
    setUserData(cleanValues.userData);
    setUserGroups(cleanValues.userGroups);
    setEmailVerified(cleanValues.emailVerified);
    setEmailVerificationCode(cleanValues.emailVerificationCode);
    setIdVerified(cleanValues.idVerified);
    setGivenName(cleanValues.givenName);
    setFamilyName(cleanValues.familyName);
    setAgeGroup(cleanValues.ageGroup);
    setGoogleLogin(cleanValues.googleLogin);
    setSocialLoginAuth(cleanValues.socialLoginAuth);
    ...
    setUpdateProfileCount(cleanValues.updateProfileCount);
    setKismetNumberDefault(cleanValues.kismetNumberDefault);
    setKismetNumberCountMax(cleanValues.kismetNumberCountMax);
    setKismetNumberCount(cleanValues.kismetNumberCount);
    setKismetNumberAvaliable(cleanValues.kismetNumberAvaliable);
    setMembersReferred(cleanValues.membersReferred);
    setReferrer0(cleanValues.referrer0);
    setReferrer1(cleanValues.referrer1);
    setDisableAccess(cleanValues.disableAccess);
    setDisableKismetDraw(cleanValues.disableKismetDraw);
    setLoginTime(cleanValues.loginTime);
    setActiveTime(cleanValues.activeTime);
    setOwner(cleanValues.owner);
    setErrors({});
  };
  const [userAccountRecord, setUserAccountRecord] =
    React.useState(userAccountModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getUserAccount.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getUserAccount
        : userAccountModelProp;
      setUserAccountRecord(record);
    };
    queryData();
  }, [idProp, userAccountModelProp]);
  React.useEffect(resetStateValues, [userAccountRecord]);
  const validations = {
    ip: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          ...
          socialLoginAuth: socialLoginAuth ?? null,
          ...
          updateProfileCount: updateProfileCount ?? null,
          kismetNumberDefault: kismetNumberDefault ?? null,
          kismetNumberCountMax,
          kismetNumberCount,
          kismetNumberAvaliable,
          membersReferred,
          referrer0: referrer0 ?? null,
          referrer1: referrer1 ?? null,
          disableAccess: disableAccess ?? null,
          disableKismetDraw: disableKismetDraw ?? null,
        ...
        try {
          ...
          await client.graphql({
            query: updateUserAccount.replaceAll("__typename", ""),
            variables: {
              input: {
      ...
      {...getOverrideProps(overrides, "UserAccountUpdateForm")}
      {...rest}
    >
      <TextField
        label="Ip"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || userAccountModelProp)}
