...
import { getRecentWinners } from "../graphql/queries";
import { updateRecentWinners } from "../graphql/mutations";
const client = generateClient();
export default function RecentWinnersUpdateForm(props) {
  const {
    id: idProp,
    recentWinners: recentWinnersModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = recentWinnersRecord
      ? { ...initialValues, ...recentWinnersRecord }
      : initialValues;
    setType(cleanValues.type);
    setTitle(cleanValues.title);
    setDescription(cleanValues.description);
    setStreamUrl(cleanValues.streamUrl);
    setShowsID(cleanValues.showsID);
    setEventsID(cleanValues.eventsID);
    setPriorityOrder(cleanValues.priorityOrder);
    setCreatedTime(cleanValues.createdTime);
    setCreatedAt(cleanValues.createdAt);
    setErrors({});
  };
  const [recentWinnersRecord, setRecentWinnersRecord] = React.useState(
    recentWinnersModelProp
  );
  React.useEffect(() => {
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getRecentWinners.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getRecentWinners
        : recentWinnersModelProp;
      setRecentWinnersRecord(record);
    };
    queryData();
  }, [idProp, recentWinnersModelProp]);
  React.useEffect(resetStateValues, [recentWinnersRecord]);
  const validations = {
    type: [],
    title: [],
    description: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateRecentWinners.replaceAll("__typename", ""),
            variables: {
              input: {
                id: recentWinnersRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "RecentWinnersUpdateForm")}
      {...rest}
    >
      <TextField
        label="Type"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || recentWinnersModelProp)}
