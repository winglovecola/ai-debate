...
import { getUserAward } from "../graphql/queries";
import { updateUserAward } from "../graphql/mutations";
const client = generateClient();
export default function UserAwardUpdateForm(props) {
  const {
    id: idProp,
    userAward: userAwardModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = userAwardRecord
      ? { ...initialValues, ...userAwardRecord }
      : initialValues;
  ...
  const [userAwardRecord, setUserAwardRecord] =
    React.useState(userAwardModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getUserAward.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getUserAward
        : userAwardModelProp;
      setUserAwardRecord(record);
    };
    queryData();
  }, [idProp, userAwardModelProp]);
  React.useEffect(resetStateValues, [userAwardRecord]);
  const validations = {
    trReferralBonus: [],
    trLoyaltyBonus: [],
    trLoyaltyBonusResetTime: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateUserAward.replaceAll("__typename", ""),
            variables: {
              input: {
                id: userAwardRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "UserAwardUpdateForm")}
      {...rest}
    >
      <TextField
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || userAwardModelProp)}
