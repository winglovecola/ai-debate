...
import { updateUserEvents } from "../graphql/mutations";
const client = generateClient();
export default function UserEventsUpdateForm(props) {
  const {
    id: idProp,
    userEvents: userEventsModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = userEventsRecord
      ? { ...initialValues, ...userEventsRecord }
      : initialValues;
    setEvent(cleanValues.event);
    setEventAction(cleanValues.eventAction);
    setEventUpdatedTime(cleanValues.eventUpdatedTime);
    setAccountUpdatedTime(cleanValues.accountUpdatedTime);
    setErrors({});
  };
  const [userEventsRecord, setUserEventsRecord] =
    React.useState(userEventsModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getUserEvents.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getUserEvents
        : userEventsModelProp;
      setUserEventsRecord(record);
    };
    queryData();
  }, [idProp, userEventsModelProp]);
  React.useEffect(resetStateValues, [userEventsRecord]);
  const validations = {
    event: [],
    eventAction: [],
    eventUpdatedTime: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          ...
          eventUpdatedTime: eventUpdatedTime ?? null,
          accountUpdatedTime: accountUpdatedTime ?? null,
        };
        ...
        try {
          ...
          await client.graphql({
            query: updateUserEvents.replaceAll("__typename", ""),
            variables: {
              input: {
                id: userEventsRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "UserEventsUpdateForm")}
      {...rest}
    >
      <TextField
        label="Event"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || userEventsModelProp)}
