...
import { getEventSession } from "../graphql/queries";
import { updateEventSession } from "../graphql/mutations";
const client = generateClient();
export default function EventSessionUpdateForm(props) {
  const {
    id: idProp,
    eventSession: eventSessionModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = eventSessionRecord
      ? { ...initialValues, ...eventSessionRecord }
      : initialValues;
  ...
  const [eventSessionRecord, setEventSessionRecord] = React.useState(
    eventSessionModelProp
  );
  React.useEffect(() => {
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getEventSession.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getEventSession
        : eventSessionModelProp;
      setEventSessionRecord(record);
    };
    queryData();
  }, [idProp, eventSessionModelProp]);
  React.useEffect(resetStateValues, [eventSessionRecord]);
  const validations = {
    eventType: [],
    rewardType: [],
    fingerPrintId: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateEventSession.replaceAll("__typename", ""),
            variables: {
              input: {
                id: eventSessionRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "EventSessionUpdateForm")}
      {...rest}
    >
      <TextField
        label="Event type"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || eventSessionModelProp)}
