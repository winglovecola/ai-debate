...
import { getShows } from "../graphql/queries";
import { updateShows } from "../graphql/mutations";
...
export default function ShowsUpdateForm(props) {
  const {
    id: idProp,
    shows: showsModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = showsRecord
      ? { ...initialValues, ...showsRecord }
      : initialValues;
    setMode(cleanValues.mode);
    setTitle(cleanValues.title);
    setDescription(cleanValues.description);
    setShowTags(cleanValues.showTags ?? []);
    setCurrentShowTagsValue("");
    setStreamUrl(cleanValues.streamUrl);
    setTapingTime(cleanValues.tapingTime);
    setBroadcastTime(cleanValues.broadcastTime);
    setPrizeHeadline1(cleanValues.prizeHeadline1);
    setPrizeHeadline2(cleanValues.prizeHeadline2);
  ...
  const [showsRecord, setShowsRecord] = React.useState(showsModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getShows.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getShows
        : showsModelProp;
      setShowsRecord(record);
    };
    queryData();
  }, [idProp, showsModelProp]);
  React.useEffect(resetStateValues, [showsRecord]);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          ...
          tapingTime: tapingTime ?? null,
          broadcastTime: broadcastTime ?? null,
          prizeHeadline1: prizeHeadline1 ?? null,
          prizeHeadline2: prizeHeadline2 ?? null,
        ...
        try {
          ...
          await client.graphql({
            query: updateShows.replaceAll("__typename", ""),
            variables: {
              input: {
                id: showsRecord.id,
                ...modelFields,
      ...
      {...getOverrideProps(overrides, "ShowsUpdateForm")}
      {...rest}
    >
      <TextField
        label="Mode"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || showsModelProp)}
