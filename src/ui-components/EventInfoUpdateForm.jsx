...
import { getEventInfo } from "../graphql/queries";
import { updateEventInfo } from "../graphql/mutations";
const client = generateClient();
export default function EventInfoUpdateForm(props) {
  const {
    id: idProp,
    eventInfo: eventInfoModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = eventInfoRecord
      ? { ...initialValues, ...eventInfoRecord }
      : initialValues;
    setPrevEventId(cleanValues.prevEventId);
    setPrevEventBroadcastTime(cleanValues.prevEventBroadcastTime);
    setLatestEventId(cleanValues.latestEventId);
    setLatestEventBroadcastTime(cleanValues.latestEventBroadcastTime);
    setNextEventId(cleanValues.nextEventId);
    setNextEventBroadcastTime(cleanValues.nextEventBroadcastTime);
    setUpcomingEventsJsonData(cleanValues.upcomingEventsJsonData);
    setUpcomingEventsJsonDataUpdateTime(
      cleanValues.upcomingEventsJsonDataUpdateTime
    );
    setTrLoyaltyBonusResetTime(cleanValues.trLoyaltyBonusResetTime);
    setErrors({});
  };
  const [eventInfoRecord, setEventInfoRecord] =
    React.useState(eventInfoModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getEventInfo.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getEventInfo
        : eventInfoModelProp;
      setEventInfoRecord(record);
    };
    queryData();
  }, [idProp, eventInfoModelProp]);
  React.useEffect(resetStateValues, [eventInfoRecord]);
  const validations = {
    prevEventId: [],
    prevEventBroadcastTime: [],
    latestEventId: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          ...
          prevEventBroadcastTime: prevEventBroadcastTime ?? null,
          latestEventId: latestEventId ?? null,
          latestEventBroadcastTime: latestEventBroadcastTime ?? null,
          nextEventId: nextEventId ?? null,
          nextEventBroadcastTime: nextEventBroadcastTime ?? null,
          upcomingEventsJsonData: upcomingEventsJsonData ?? null,
          upcomingEventsJsonDataUpdateTime:
            upcomingEventsJsonDataUpdateTime ?? null,
          trLoyaltyBonusResetTime: trLoyaltyBonusResetTime ?? null,
        };
        ...
        try {
          ...
          await client.graphql({
            query: updateEventInfo.replaceAll("__typename", ""),
            variables: {
              input: {
                id: eventInfoRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "EventInfoUpdateForm")}
      {...rest}
    >
      <TextField
        label="Prev event id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || eventInfoModelProp)}
