...
import { getPrizesType } from "../graphql/queries";
import { updatePrizesType } from "../graphql/mutations";
const client = generateClient();
export default function PrizesTypeUpdateForm(props) {
  const {
    id: idProp,
    prizesType: prizesTypeModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = prizesTypeRecord
      ? { ...initialValues, ...prizesTypeRecord }
      : initialValues;
  ...
  const [prizesTypeRecord, setPrizesTypeRecord] =
    React.useState(prizesTypeModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getPrizesType.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getPrizesType
        : prizesTypeModelProp;
      setPrizesTypeRecord(record);
    };
    queryData();
  }, [idProp, prizesTypeModelProp]);
  React.useEffect(resetStateValues, [prizesTypeRecord]);
  const validations = {
    title: [{ type: "Required" }],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updatePrizesType.replaceAll("__typename", ""),
            variables: {
              input: {
                id: prizesTypeRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "PrizesTypeUpdateForm")}
      {...rest}
    >
      <TextField
        label="Title"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || prizesTypeModelProp)}
